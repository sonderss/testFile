...
<view class="list_box"  style="overflow: hidden;"  @touchmove.stop.prevent="moveHandle" >
 ...
 -->				
				<list  style="width: 100%;height: 800rpx;">
						  ...
						  <cell style='display: block;' v-for="(item,index) in mainArray" :key="item.id" class="item" @tap="leftTap(index)" :class="{ 'active':index==leftIndex }" :data-index="index" v-if="item.product && item.product.length >0">{{item.cate_name}}</cell>
				</list>
